...
class DMPkg(object):
    ...
    def xmlHeader(self):
        ...
        hstr += 'xsi:schemaLocation="https://www.s3model.com/ns/s3m/  https://dmgen.s3model.com/dmlib/dm-' + str(self.dm.ct_id).strip() + '.xsd">\n'
        return(hstr)
    ...
    def processCluster(self, cluster):
        ...
        for clust in cluster.clusters.all():
            ...
            if self.clusters.count(clust.ct_id) > 100:
                self.msg = ("I think the DMGEN is in a loop because you have embedded a Cluster inside itself on some level, OR there are more than 100 embedded Clusters, which seems kind of ridiculous.", messages.ERROR)
                return(self.msg)
        ...
        for Xd in cluster.xdcount.all():
            if self.registerUUID(Xd.ct_id, 'XdCountType', 'XdAdapter-value', Xd.label):
                ...
                if Xd.units:
                    ...
                    self.msg = ("Your XdCount is missing a Xdcount-units value. This should have been a publishing error; <b>Contact the DMGEN authors</b>.", messages.ERROR)
                    return(self.msg)
            if not (Xd.adapter_ctid in self.adapters):
                # create the XdAdapterType code
                self.xsd += self.makeXdAdapter(Xd.ct_id, Xd.adapter_ctid, Xd.label)
        ...
        for Xd in cluster.xdquantity.all():
            if self.registerUUID(Xd.ct_id, 'XdQuantityType', 'XdAdapter-value', Xd.label):
                ...
                if Xd.units:
                    ...
                    self.msg = ("Your XdQuantity is missing a Xdquantity-units value. This should have been a publishing error; <b>Contact the DMGEN authors</b>.", messages.ERROR)
                    return(self.msg)
            if not (Xd.adapter_ctid in self.adapters):
                # create the XdAdapterType code
                self.xsd += self.makeXdAdapter(Xd.ct_id, Xd.adapter_ctid, Xd.label)
