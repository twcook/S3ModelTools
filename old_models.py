...
class Common(models.Model):
    ...
    label = models.CharField(_('label'), max_length=110, help_text=_("A human readable label used to identify this model in DMGEN. This will also be use in generated app UI."))
    ...
    schema_code = models.TextField(_("Schema Code"), help_text="This is only writable from the DMGEN, not via user input. It contains the code required for each component to create an entry in a DM.", blank=True, null=True, default='')
    ...
    app_code = models.TextField(_("App Code"), help_text="This is only writable from the DMGEN, not via user input. It contains the code required for each component to create the User App.", blank=True, null=True, default='')
...
class DM(models.Model):
    ...
    about = models.CharField(_('About'), default="http://dmgen.s3model.com/dmlib/", max_length=255, help_text=_("The URL to the DM after publication. The DM ID will be added after the trailing slash in the format of 'dm-{dm_id}.xsd' This provides a full path and filename for the dm as a unique identifier."), blank=True)
    ...
    dc_language = models.CharField(_("DC Language"), max_length=6, default="en-US", choices=LANGUAGES, help_text=_('The written language of the DM.'), blank=True)
    ...
    schema_code = models.TextField(_("Schema Code"),  help_text="This is only writable from the DMGEN, not via user input. It contains the code required for each component to create an entry in a DM.", default='', blank=True, null=True, editable=True)
    doc_code = models.TextField(_("Documentation Code"), help_text="This is only writable from the DMGEN, not via user input. It contains the HTML code to document the DM.", null=True,  blank=True)
    app_code = models.TextField(_("App Code"), help_text="This is only writable from the DMGEN, not via user input. It contains the code required to create the User App.", blank=True, null=True, default='')
